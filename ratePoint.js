;(function($){
  $.fn.animeRateBar = function(options){
   ...
   return this.each(function(index,item){
      ...
      // later
      // return setTimeout($.proxy(function(e){          
      //     return loadProgress();
      //   }, this),1000)
      // if(isNaN(oSettings.delay) && typeof oSettings.delay != "boolean"){ 
      //   return loadProgress();
      // }else{
      //   return setTimeout($.proxy(function(){          
      //      return loadProgress();
      //   }, this),oSettings.delay)
      // }
  ...
  function loadInstance(){
    ...
    if(!isPathTag){
      ...
      if(animVal != null && animVal != ""){   
       ...
       rateBar.attr({[animVal]: startRate+","+xBar});
       rateBox.html(loadPoint+textPercent+textUnit+textLabel);
      }
    }
    return this;
    ...
    jQuery({ Counter: startPoint}).animate({ Counter: endPoint },
     {
       ...
       step: function(){
         ...
         if(isPathTag){
           ...
           //console.log($.fn.animeRateBar.defaults);
           rateBar.attr({[animVal]: $startPoint+","+cBar});
         }else{
         ...
         var $textwrite = isFlat ? nstepCounter+textPercent+textUnit+textLabel : realendPoint+textPercent+textUnit+textLabel;
         ...
         if(textLabel.indexOf("text:") >= 0){ 
           ...
           //show only text
           rateBox.html('<xhtml:span id="xhtml" class="mybox">'+textContent+'</xhtml>');
         }else{
           if(textLabel == "rate"){ 
             //show only rate 
             $textwrite = isFlat ? nstepCounter+textPercent+textUnit : realendPoint+textPercent+textUnit;
             rateBox.html('<xhtml:span id="xhtml" class="mybox"><span class="mybox xid">'+$textwrite+'</span><span class="mybox-full"></span></xhtml>');
           }else if(textLabel == "none"){
             //show none
             rateBox.html('<xhtml:span id="xhtml" class="mybox"></xhtml>');
           }else{
             //show both
             //if(textLabel != null){
             var textCon = (textLabel == "")? '<span class="mybox xid">'+$textwrite+'</span><span class="mybox-full"></span>' : $textwrite;
              rateBox.html('<xhtml:span id="xhtml" class="mybox">'+textCon+'</xhtml>');
             //}
            }
         }
                 ...
                 if(breakVal <= nthRater){
                    if(breakClasses != null){
                      ...
                      if($(item).find('[data-toggle]').length > 0){
                         if(toggleAttr != null){
                          $(item).find('[data-toggle]').attr({[toggleAttr]:obClass});
                         }
                      }
                     }
                    }
...
function radialRateInitiate(ratePoint){
  ...
  if($(item).data('self') && item.data('point')){ console.log("running");
  ...
  var objectItem = {runtime:runtime,        
...
;(function($){
  ...
  function runRate(settings){
    ...
    var rateBar   = item.find(".rate-bar");
    var rateBox = item.find(".rate-value");
    var dataRate  = rateBox.attr('data-rate'); //endPoint
    ...
    jQuery({ Counter: startPoint}).animate({ Counter: dataRate},
     {
       ...
       step: function(){
         ...
         //dynamic variables
         var progressPane = item;
         var progressPath = item.find(".rate-bar").parent();
         var progressBar  = item.find(".rate-bar")

         var pathWidth = progressPath.width();
         var progressRate = progressBar.width();

         var percentFill  = Math.ceil(this.Counter)+"%";
         var textFill     = this.Counter.toFixed(2)+"%";
         var realRate = (progressBar.width() / (pathWidth - 4)) * 100;                      
